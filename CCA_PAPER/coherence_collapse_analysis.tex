...
\begin{enumerate}
    ...
    \item A \textbf{stability condition} ($\alpha/k \geq d$) that distinguishes growing from decaying systems.

    \item Formal verification of key theorems in \textbf{Lean 4} with Mathlib dependencies.

    \item \textbf{Hardware instrument} (C-series): a 128-sensor GPU-timing strain gauge, characterized; first measurement of correlation propagation velocity ($0.5$ m/s). \emph{The v3 claim of ``empirical validation of the $\keff$ formula ($R^2 = 0.798$)'' is withdrawn} --- that comparison is an identity check (Remark~\ref{rem:c1-identity}).
...
\paragraph{Theorem~\ref{thm:volume-decay} substitutes $\keff$ into the exponent without deriving the step.}
...
values of 5.20 and 7.5. Everything downstream inherits the step: $\Ttruth$, the singularity
boundary $\Kreq \cdot \rho \geq 1$, and the phase corridor. This is flagged, not repaired, and
carries the same status as every other unproven item corrected in this version: it is an
\textbf{open} step, not a proved one.

\paragraph{The deceptive/honest asymmetry is assumed, not derived.}
The volume-decay theorem cuts a bounded convex region with independent halfspaces. Nothing in
that construction distinguishes the deceptive region from the honest one: \emph{both} shrink as
constraints accumulate. What safety requires is that the \emph{ratio} fall --- that the
deceptive region shrink faster than the honest one --- and that holds only if the constraints
are chosen so that honest behaviour satisfies them while deceptive behaviour does not. That
choice is the alignment problem itself, assumed here rather than solved. The limitation
previously stated in this paper (``assumes convex deceptive regions'') is a weaker and different
caveat, and does not cover this. Stated plainly: \textbf{the geometry shows that constraints
shrink feasible volume; it does not show that they shrink deception preferentially.}
