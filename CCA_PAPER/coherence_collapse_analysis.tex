...
\textbf{Three of the six have now fired against the framework; two are unmet; one passes.}
...
\textbf{One caveat, which cuts against this result and is stated because it is real.}
Perturbation response conflates two distinct things: a system that is genuinely robust, and a
sensor array that has stopped discriminating. At $\rho \to 1$ the array approaches a single
effective sensor, and a single sensor responds less to spatial perturbation regardless of
whether it is robust. So a falling response above the upper edge is consistent both with
``rigidity is not fragile here'' and with ``the fragility measure stops working exactly where
the corridor claims to need it.'' The F-series used this measure to establish the chaos arm, so
the objection applies to that result too. Resolving it requires a fragility measure that does
not degrade with $\keff$ --- recovery time after perturbation, or detection latency at fixed
false-positive rate --- and that measurement is not supplied here. The condition is reported as
fired under its pre-registered rule, with the measure's limitation on the record.

The chaos arm was not re-tested: no run reached $\rho < 0.10$ (minimum achieved $0.122$), so
condition 3's chaos-side prediction remains untested rather than replicated.
