...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Threshold transfer.} The collapse rule $\sigma < \sigma_{\min}$ or $f > f_{\max}$ carries two free parameters, fitted per domain. Fix one pair on one domain and apply it unchanged to another. If classification accuracy does not exceed a base-rate-matched null under a pre-specified outcome definition, the rule is a per-domain fitted classifier rather than a structural invariant. \emph{Currently unmet}: no threshold pair in this paper has been shown to transfer, and the one domain scored against a pre-specified outcome (\S\ref{sec:institutional-negative}) performs below chance.
    \item \textbf{Sensitivity calibration.} The detection model $P = 1 - e^{-\beta\varepsilon}$ has a free $\beta$, never independently calibrated. Estimate $\beta$ on one substrate and predict detection rates on a second. If $\beta$ must be refit per substrate, the model has no predictive content beyond curve-fitting, and quantities derived from a chosen $\beta$ --- including the ``$\sim 40\%$'' undetectable fraction of earlier versions --- carry none either. \emph{Currently unmet}: $\beta = 10$ is a stipulation, and \S\ref{sec:mc-selfcheck} shows the only test of it samples from the model being tested.
\end{enumerate}

\paragraph{Conditions owed by the fires.}
A fired condition that leaves no successor lets the repaired claim return unchallenged. Three
more are therefore entered, each generated by a specific result above.

\begin{enumerate}[resume]
    \item \textbf{Replacement decay law.} Condition 4 fired, so $V = V_0 e^{-\lambda\keff}$ is not the correct law. Any proposed replacement must (a) reduce to $k$ at $\rho = 0$, (b) reproduce the measured over-prediction of effective count, $1.9\times$ to $4.1\times$ at $\rho = 0.5$ growing in $k$, and (c) account for the degradation observed at large $k$ even at $\rho = 0$ (implied count $25.7$ against $k = 32$). A law failing any of the three is not the repair, whatever else it fits.
    \item \textbf{Sign of the effective-count residual.} Condition 1 passed at $20.4\%$, but $k/\lambda_{\max} \leq \keff$ in $21$ of $21$ matrices --- a one-directional bias, not noise. A corrected estimator must eliminate the residual's \emph{sign}, not merely shrink its magnitude. A proposal that halves the disagreement while leaving it systematically signed has fitted the size of the error without finding its mechanism.
    \item \textbf{Prior validation of fragility measures.} Any measure used to support a corridor claim must be shown $\keff$-independent \emph{before} it is used, on data spanning the corridor. Three measures have now failed this check after the fact (\S\ref{sec:f1f3-results}). Post-hoc justification of a fragility measure is not admissible for corridor claims in this framework.
\end{enumerate}

\textbf{Of nine conditions: two fired, three are unmet, one is unresolved, one passes, and three
are newly entered by the fires.}
That distribution is reported rather than smoothed, because a condition that cannot fire is not
a falsification condition, and the three carried in version 3 could not. They are withdrawn
rather than reworded.

\subsection{Results of conditions 1 and 3}
\label{sec:f1f3-results}

Both were runnable on data and hardware already in hand, so they were run rather than left
standing. Pre-registered before either quantity was computed (hash and predictions in
\texttt{replication/}).

\paragraph{Condition 1 --- does not fire, but the residual is systematic.}
Over the 21 stored C1 correlation matrices ($k=16$), median disagreement between
$k/\lambda_{\max}$ and $\keff$ is $\mathbf{20.4\%}$ on the precondition-satisfying set
($\rho \geq 0.04$), against a $25\%$ bar. \textbf{Kish passes this test} --- the first
non-circular check of the identity anywhere in this corpus, and the measurement
\S\ref{sec:kish-implementation} says the paper lacks.

Two features of the residual matter more than the pass. First, the direction is not random:
$k/\lambda_{\max} \leq \keff$ in \textbf{21 of 21} matrices, so the identity systematically
\emph{over-credits} effective diversity relative to the spectrum --- the unsafe direction, since
a system reading $\keff$ believes it has more independent constraints than the eigenstructure
supports. Second, the disagreement concentrates where the precondition is weakest: median
$23.2\%$ for $\rho < 0.10$ against $11.1\%$ for $\rho \geq 0.20$, consistent with the Exp 117
finding that exchangeability fails at low correlation. A $20.4\%$ median against a $25\%$ bar is
a pass, not a comfortable one.

\paragraph{Condition 3 --- fires. The rigidity arm is not observed.}
The rigidity regime was reached for the first time on this substrate ($\rho$ up to $0.868$,
against the F-series maximum of $0.171$). Perturbation response, the F-series fragility measure:

\begin{center}
\begin{tabular}{lrrr}
\toprule
Regime & $n$ & mean response & sd \\
\midrule
healthy ($0.10 \leq \rho < 0.43$) & 7 & $36.7\%$ & $17.5\%$ \\
rigidity ($\rho \geq 0.43$) & 5 & $\mathbf{10.0\%}$ & $5.2\%$ \\
\bottomrule
\end{tabular}
\end{center}

Fragility does not rise above the upper edge --- it \emph{falls}, by a factor of $3.7$. Under
the pre-registered rule the condition fires: \textbf{the corridor is not two-sided on this
substrate.} Combined with Remark~\ref{rem:corridor-arm}, the position is that the chaos arm is
supported and the rigidity arm is now measured and absent.

\textbf{That firing does not stand, and the reason is instructive.} Perturbation response
conflates two distinct things: a genuinely robust system, and a sensor array that has stopped
discriminating. At $\rho \to 1$ the array approaches a single effective sensor, and a single
sensor responds less to spatial perturbation whether or not it is robust. So the falling
response is equally consistent with ``rigidity is not fragile here'' and with ``the fragility
measure stops working exactly where the corridor needs it.'' The F-series used this same measure
for the chaos arm, so the objection reaches that result too.

\paragraph{Condition 3, re-run under recovery time --- unresolved.}
The objection was addressed by building the measure it calls for. Recovery time $\tau$ is
estimated from the AR(1) lag-1 autocorrelation of the array mean after a sustained perturbation
is released --- the standard critical-slowing-down statistic, and a \emph{temporal} property, so
it does not degrade as the array loses spatial discrimination. Sampling is synchronous in every
condition (time-resolved array state requires it), and $\rho$ is varied by memory sharing rather
than by stream asynchrony, so the correlation control is independent of the sampling method.

\begin{center}
\begin{tabular}{lrrrr}
\toprule
Regime & $n$ & $\tau$ (array) & sd & $\tau_{\text{array}}/\tau_{\text{single}}$ \\
\midrule
healthy ($0.10 \leq \rho < 0.43$) & 4 & $2.21$ & $1.17$ & $2.15$ \\
rigidity ($\rho \geq 0.43$) & 12 & $3.67$ & $2.82$ & $1.32$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{The two measures disagree in direction.} Recovery time is \emph{longer} at rigidity
($+1.46$), the sign critical slowing down predicts and the opposite of what response amplitude
showed --- which is what one expects if the first result was carrying the confound. But the
difference is smaller than the pooled standard deviation ($2.29$), so it is not significant; the
healthy bin holds only 4 of 16 runs because achieved $\rho$ landed high; and the pre-registered
control failed: the array-to-single-sensor $\tau$ ratio reached $2.15$ in the healthy bin
against an admissibility bound of $2.0$, so this measure cannot be certified free of its own
$\keff$ dependence either.

Under the pre-registered decision rule a failed control makes the condition
\textbf{UNRESOLVED}, and neither the earlier firing nor a survival may be claimed.
\textbf{The version-3 corridor claim therefore remains unsupported on this substrate, and the
first run's apparent refutation of it is withdrawn as well.} What is established is narrower and
worth stating on its own: the rigidity regime is reachable here ($\rho$ to $0.868$), two
defensible fragility measures give opposite signs across the upper edge, and neither is clean.
\paragraph{Condition 3, third attempt --- and why it cannot be settled on this instrument.}
Detection latency at fixed false-positive rate was the remaining candidate, and it was built:
the array is a detector, so fragility is degraded detection, with the decision threshold
calibrated per condition on that condition's \emph{own} baseline to a $5\%$ false-positive rate.
That calibration was the point --- it normalises away the variance scale that contaminated both
earlier measures.

It failed, for a reason worth recording. At \emph{zero} perturbation the measure reports a
latency of $3.3$ samples where its own $5\%$ calibration implies roughly $20$: the array's
baseline drifts between calibration and test, so the detector is timing drift rather than
signal. Perturbation response is also a cliff rather than a gradient --- latency sits near the
false-alarm floor across a range of drive strengths and then collapses to $1.0$ --- leaving no
dynamic range in which conditions could be compared.

The three failures share one cause. \textbf{Every $\keff$-independent fragility measure requires
the array state at each time index, which requires synchronous sampling across all sensors; and
synchronous sampling on this instrument is itself a correlating operation, holding $\rho$ at or
above $0.45$ in every condition attempted.} Varying workload heterogeneity to break the coupling
raised $\rho$ further ($0.468 \to 0.713$) rather than lowering it. The healthy band is therefore
not reachable by any time-resolved measure on this hardware, and the comparison the corridor
claim needs cannot be constructed here.

\textbf{F-3 is settled as unsettleable on this substrate, which is a property of the instrument
rather than of the corridor.} Deciding it requires an instrument in which correlation varies
independently of sampling synchrony --- physically separated sensors across multiple devices, or
asynchronous per-sensor sampling interpolated onto a common time grid. Until then the corridor's
rigidity arm is neither supported nor refuted, and this paper claims neither.

The chaos arm was not re-tested under either measure: no run reached $\rho < 0.10$, so
condition 3's chaos-side prediction is untested rather than replicated.

Two claims that appeared in version 3's ``structural invariants'' receive no falsification
handle here, because none can exist for them. $\keff = k/(1+\rho(k-1))$ is an \textbf{identity};
$f = 1 - \sigma$ is a \textbf{definition} in the engines that report it as validated. Neither
can be contradicted by any measurement, and the appropriate response is to stop describing them
as invariants that were checked --- which this version does --- rather than to invent a
falsifier they cannot have.
...
\subsection{Monte Carlo Self-Consistency Check}
\label{sec:mc-selfcheck}

Version 3 reported this table as Monte Carlo \emph{validation} of the detection probability
model. It is not a validation, and the label is corrected here. The simulation draws detection
events with probability $1 - e^{-\beta\hat\varepsilon}$ from a noisy estimate $\hat\varepsilon$
and compares the resulting rate against $1 - e^{-\beta\varepsilon}$ --- the same expression at
the noiseless value. The two cannot disagree except through the concavity of $1-e^{-\beta\cdot}$
in its argument, so the residual column measures a Jensen bias, not model adequacy. The
closed-form prediction for that bias, $-\tfrac{1}{2}\beta^2 e^{-\beta\varepsilon}\sigma^2$,
reproduces the observed differences: $-1.21\%$, $-0.80\%$, $-0.45\%$, $-0.27\%$ against the
tabulated $-0.4\%$, $-0.9\%$, $-0.5\%$, $-0.4\%$. \textbf{No independent detection process is
involved, and the reported RMSE of 0.006 measures how faithfully the sampler reproduces its own
sampling distribution.} This is the third instance in this paper of a residual that is a Jensen
gap rather than a measurement error (cf.\ Remark~\ref{rem:c1-identity}).

What the table legitimately shows is that the implementation samples from the model it claims
to sample from, with $\beta = 10$ and measurement noise $\sigma = 0.02$.
...
\bottomrule
\end{tabular}
\caption{Monte Carlo self-consistency check, $\beta=10$. \textbf{Not a validation:} the ``empirical'' column is sampled from the same expression the ``theoretical'' column evaluates. The difference column is the concavity bias of the noise in $\hat\varepsilon$, matching the closed-form Jensen prediction, and the RMSE of 0.006 measures sampler fidelity rather than model adequacy.}
\end{table}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{WITHDRAWN.} v3 reported ``$f$ validation: computed $f = 20.69\% = 1-\sigma$ (exact match).'' In the engine $f$ is \emph{defined} as $1-\sigma$ (\texttt{battery.py:427} returns \texttt{1 - self.get\_sigma()}; the neural engine assigns \texttt{f = max(0, 1.0 - sigma)}), so the ``exact match'' reports that $1-\sigma = 1-\sigma$. This is a definition, not a validation, and no measurement could have contradicted it
...
Coherence Collapse Analysis models coherence through constraint geometry. What that yields,
after this version's corrections and the falsification conditions run against it, divides
cleanly.

\paragraph{What is established.}
The effective-count map $\keff = k/(1+\rho(k-1))$ is an identity with a M\"obius structure:
strictly monotone in $\rho$, saturating at $1/\rho$. Scale cannot restore diversity that
correlation has collapsed, and this holds without calibration on any substrate. The stability
condition $\alpha/k \geq d$ and the three closed-form collapse timescales follow from stated
assumptions and are machine-verified in Lean --- which establishes internal consistency and, as
\S\ref{sec:formal-verification} says, nothing about the world. L-01 establishes that the class of
marginal-preserving undetectable patterns is non-empty: existence, no measure. The GPU array is a
real, characterized instrument, and its measurement protocol --- shuffle null, repeated trials,
first-run discard --- is transferable.

\paragraph{What fired against it.}
Of six falsification conditions, two fired. The equicorrelated precondition the identity requires
\emph{fails} below $\rho \approx 0.04$ on the measured substrate, so $\keff$ figures from that
band are unlicensed. And the volume-decay law $V = V_0 e^{-\lambda \keff}$ is wrong by a factor of
$1.9$ to $4.1$ at $\rho = 0.5$, with the error growing in $k$ --- so $\Ttruth$, the singularity
boundary, and the phase corridor all rest on a step that does not hold. Two further conditions are
unmet, one is unresolved, and one passes.

\paragraph{What is not established, and was claimed in version 3.}
Cross-domain validation: withdrawn --- applying one formula to three datasets shows it can be
applied, not that it holds. Hardware confirmation of the $\keff$ formula: withdrawn, twice over,
as both results computed the identity and compared it against itself. Software-induced collapse:
withdrawn as a measurement artifact. The two-sided corridor: unresolved, and \S\ref{sec:f1f3-results}
reports why three separate fragility measures could not settle it. The deceptive/honest asymmetry
that would make any of this a safety result is assumed, not derived, and simulation shows that
under the theorem's own assumptions adding constraints buys no differential protection at all.

\medskip
\noindent\textbf{Final assessment.} CCA is a \textbf{vocabulary and a measurement substrate for
correlated fragility}, not a validated predictive framework. Its durable content is an identity
with a saturation ceiling --- inherited from Kish (1965) and reached independently in four other
fields --- together with an instrument for measuring correlation structure and a discipline for
deciding when such measurements mean anything. Its central safety intuition, that accumulating
independent constraints starves deception, does not follow from the geometry presented here: the
geometry shows that constraints shrink feasible volume, not that they shrink deception
preferentially. Making hidden fragility legible remains the goal; this version establishes how
much of that goal the evidence currently supports, which is less than version 3 claimed and more
than nothing.
